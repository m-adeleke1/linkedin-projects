...
provider "azurerm" {
  features {}
  subscription_id = "dcf32054-c1fe-4e23-995a-89c1ebc7ed29"  # e.g. 80ea84e8-afce-4851-928a-9e2219724c69
...
resource "azurerm_linux_web_app" "calculator_app" {
  name                = "mcp-calculator-app-12345"  # Replace [random_id] with a unique suffix
  location            = data.azurerm_resource_group.existing_rg.location
  resource_group_name = data.azurerm_resource_group.existing_rg.name
  service_plan_id     = azurerm_app_service_plan.plan.id
